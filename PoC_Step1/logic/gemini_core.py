    ...
    def upload_file(self, path, mime_type=None):
        ...
        if ext in ['.xlsx', '.xls']:
            try:
                import pandas as pd
                import tempfile
                print(f"Converting Excel {os.path.basename(path)} to CSV for Gemini...")
                ...
                print(f"Warning: Excel conversion failed: {e}. Uploading original.")
        ...
        print(f"Uploading {os.path.basename(path_to_upload)} (MIME: {mime_type})...")
