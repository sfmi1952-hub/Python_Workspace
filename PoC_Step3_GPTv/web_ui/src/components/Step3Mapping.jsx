...
export default function Step3Mapping() {
    ...
    const ConfidenceBadge = ({ val }) => {
        ...
        if (!val) return <td className="p-3 border-b border-gray-800"><span className="text-gray-700 text-xs">-</span></td>
        return (
            <td className="p-3 border-b border-gray-800">
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border ${styles[val] ?? "bg-gray-800 text-gray-400 border-gray-700"}`}>
                    {val}
                </span>
            </td>
        )
    ...
    const SourceBadge = ({ val }) => {
        const label = {
            ...
            policy_text:       "약관본문",
            mapping_table:     "매핑테이블",
            external_knowledge:"외부지식",
        }[val] ?? val
        const styles = {
            appendix:          "bg-blue-900/40 text-blue-300",
            policy_text:       "bg-indigo-900/40 text-indigo-300",
            mapping_table:     "bg-purple-900/40 text-purple-300",
            external_knowledge:"bg-gray-800 text-gray-400",
        }
        if (!val) return <td className="p-3 border-b border-gray-800"><span className="text-gray-700 text-xs">-</span></td>
        return (
            <td className="p-3 border-b border-gray-800">
                <span className={`px-2 py-0.5 rounded text-[10px] font-medium ${styles[val] ?? "bg-gray-800 text-gray-400"}`}>
                    {label}
                </span>
            </td>
        )
            ...
            <header className="flex items-center justify-between mb-6 pb-4 border-b border-gray-800">
                <div>
                    <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-400 to-indigo-400 text-transparent bg-clip-text flex items-center gap-3">
                        ...
                        PoC Step 3: Multi-Attribute Inference (GPT-5.2)
                    </h1>
                    <p className="text-gray-400 text-sm mt-1">
                        OpenAI GPT-5.2 + Responses API — Infer 9 Attributes using Multiple Mapping Tables
                    </p>
                </div>
                ...
                <div className="flex-1 bg-gray-900 rounded-xl border border-gray-800 flex flex-col overflow-hidden shadow-2xl">
                    <div className="px-4 py-3 border-b border-gray-800 bg-gray-900/80 flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                            {downloadPath && (
                                <button
                                    onClick={() => handleDownload(downloadPath, "All_Results.zip")}
                                ...
                                >
                                    <Download size={14} /> All Results (ZIP)
                                </button>
                            )}
                        </div>
                    </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-gray-800 text-xs uppercase text-gray-400 font-semibold sticky top-0 z-10 shadow-lg">
                                    <tr>
                                        ...
                                        <th className="p-3 border-b border-gray-700 min-w-[90px] text-green-300">Confidence</th>
                                        <th className="p-3 border-b border-gray-700 min-w-[90px] text-indigo-300">Source</th>
                                        <th className="p-3 border-b border-gray-700 min-w-[60px]">Page</th>
                                        <th className="p-3 border-b border-gray-700 min-w-[320px]">Logic / Reason</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-800 text-sm">
                                    {result.map((row, i) => (
                                        <tr key={i} className="hover:bg-gray-800/50 transition-colors">
                                            ...
                                            <ConfidenceBadge val={row['Confidence']} />
                                            <SourceBadge val={row['Source']} />
                                            <td className="p-3 border-b border-gray-800 text-gray-400 text-xs text-center">
                                                {row['Ref_Page'] || <span className="text-gray-700">-</span>}
                                            </td>
                                            <td className="p-3 border-b border-gray-800 text-gray-500 italic text-[10px] max-w-[320px] truncate" title={row['Code_Mapping_Reason']}>
