# run_poc.ps1
Write-Host "Started PoC Step 3 Environment (Unified Diagnosis & Attributes)" -ForegroundColor Cyan
...
if (Test-Path $VenvPython) {
    Start-Process cmd -ArgumentList "/k title PoC Step 3 Backend (8002) && $VenvPython server.py" -WorkingDirectory "$PSScriptRoot"
}
else {
    Write-Warning "Venv not found at $VenvPython, trying global python..."
    Start-Process cmd -ArgumentList "/k title PoC Step 3 Backend (8002) && python server.py" -WorkingDirectory "$PSScriptRoot"
}
