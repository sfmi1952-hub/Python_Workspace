...
class DiagnosisMapper:
    ...
    def _parse_json_with_llm_retry(self, raw_text, logger=print):
        ...
        try:
            fix_resp = self.client.generate_content(
                [fix_prompt], request_options={"timeout": 120}
            )
            result = self._parse_json_response(fix_resp.text, logger)
            if result:
    ...
    # ─────────────────────────────────────────────────────────────────────────
    def _safe_delete_file(self, file_obj):
        """google.generativeai SDK 버전 차이를 처리하는 파일 삭제 헬퍼."""
        try:
            # SDK 0.8+ : genai.files.delete(name=...)
            if hasattr(genai, "files") and hasattr(genai.files, "delete"):
                genai.files.delete(name=file_obj.name)
            # 구버전 SDK : genai.delete_file(name)
            elif hasattr(genai, "delete_file"):
                getattr(genai, "delete_file")(file_obj.name)
        except Exception:
            pass  # 삭제 실패는 치명적이지 않으므로 무시
...
"""
    ...
    def process(self, target_pdf, target_excel, mapping_files, ref_files=[], logger=print):
        try:
            ...
            # 타겟 PDF 업로드 (1회)
            t_pdf = self.client.upload_file(
                target_pdf, mime_type="application/pdf", logger=logger
            )
            ...
            for idx, config in enumerate(ATTRIBUTE_CONFIGS):
                        ...
                        try:
                            ...
                            try:
                                for p in paths:
                                    ...
                                    group_files_objs.append(f)

                                rule_resp = self.client.generate_content(
                                    [rule_prompt] + group_files_objs,
                                    request_options={"timeout": 600},
                                )

                                if rule_resp.text and "관련 내용 없음" not in rule_resp.text:
                                    # JSON 구조 파싱 시도
                                    parsed_logic = self._parse_json_response(
                                        rule_resp.text, logger
                                    )
                                    if parsed_logic:
                                ...
                                logger(f"  > PDF 업로드 실패 ({api_err}). 텍스트 폴백...")
                            ...
                            for gf in group_files_objs:
                                self._safe_delete_file(gf)
                            time.sleep(5)
                ...
                batch_results = []
                try:
                    req_content = [prompt, t_pdf]
                    resp = self.client.generate_content(
                        req_content, request_options={"timeout": 1000}
                    )

                    logger(
                        f"  > [DEBUG] 응답 첫 500자: {resp.text[:500] if resp.text else '(empty)'}"
                    )

                    # [개선 #7] 강건한 JSON 파싱 + LLM 재시도
                    batch_results = self._parse_json_with_llm_retry(resp.text, logger)
    ...
    def _phase1_text_fallback(self, paths, rule_prompt, extracted_logics, group_name, logger):
        ...
        if fallback_text:
            try:
                fb_resp = self.client.generate_content(
                    [rule_prompt, fallback_text], request_options={"timeout": 600}
                )
