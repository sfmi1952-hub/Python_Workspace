...
class TagProcessor:
    ...
    def _range_process_output_control_tags(self, range_obj, context: TagContext, cached_text=None, log_callback=None):
        ...
        try:
            ...
            # List of output control tag patterns to process
            tag_configs = [
                ('면책0', context.면책 > 0),
            ...
            # Process N-based tags (담보코드 순번별) - reduced to 10
            for n in range(1, 10):
                tag_configs.extend([
                    (f'감액있음{n}', context.감액 > 0),
                    (f'비갱신{n}', context.비갱신 > 0 or context.자동갱신형 == 0),
                    (f'갱신{n}', context.자동갱신형 > 0),
                    (f'감액한번{n}', context.감액한번),
                    (f'감액두번{n}', context.감액두번),
                    (f'진단확정{n}', context.진단확정 > 0),
                    (f'자동갱신형{n}', context.자동갱신형 > 0),
                ])
            
            # ONLY process tags that exist in the range
        ...
        try:
            ...
            for find_text, replace_text in replacements.items():
                try:
                    ...
                    if log_callback:
                        print(f"      [DEBUG-EXT] Replacing '{find_text}' | Range: {search_range.Start}~{search_range.End}")
                    ...
                    while replace_count < max_loop:
                        ...
                        if log_callback:
                            print(f"      [DEBUG] Manual Replaced instance {replace_count}")
                    ...
                    if log_callback:
                        print(f"      [DEBUG] Error replacing {find_text}: {e}")
                    pass
                finally:
                    pass
                    find.MatchWildcards = False
